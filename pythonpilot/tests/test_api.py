...
@pytest.mark.dependency(depends=['test_get_db_info'])
def test_filter_props_rw(client):
    data = api.list_properties(pytest.initial_props, OBJ_ID)
    rw_props = api.filter_props(data, filter=api.Permission.RW)
...
async def test_set_property(client):
    ...
    data = api.list_properties(await api.get_db_information(client, pytest.session), OBJ_ID)
